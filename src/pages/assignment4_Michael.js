...
const Charts = () => {
    ...
    .domain(data.map(d => d.station)) // 站点名称作为 x 轴的域
    .range([0, innerWidth])  // x 轴的范围是从 0 到图表的宽度
    .padding(0.1); // 调整条形之间的间距

// 使用 d3.scaleLinear() 来为 y 轴（条形的高度）创建比例尺
    const yScaleBar = d3.scaleLinear()
    .domain([0, d3.max(data, d => d.tripdurationS)]) // y 轴的范围是 tripdurationS 的最大值
